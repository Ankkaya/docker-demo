 ...
 *
 * Adapted as an in-repo utility for uni-app runtime usage.
 */
...
export function createQrMatrix(content: string, level: keyof typeof QRErrorCorrectLevel = 'L') {
  const code = new QRCode(-1, QRErrorCorrectLevel[level] ?? QRErrorCorrectLevel.L)
  ...
  for (let row = 0; row < moduleCount; row += 1) {
    const line: boolean[] = []
    for (let col = 0; col < moduleCount; col += 1) {
      line.push(Boolean(code.isDark(row, col)))
    }
    matrix.push(line)
  }
