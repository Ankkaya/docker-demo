<template>
  <div class="p-4">
    ...
    <!-- 上传记录列表 -->
    <n-card>
      <n-data-table :columns="columns" :data="recordList" :loading="loading" :pagination="pagination"
        remote @update:page="handlePageChange" @update:page-size="handlePageSizeChange" />
    </n-card>
  </div>
</template>
...
const columns: DataTableColumns<UploadRecord> = [
  ...
  { title: '用户ID', key: 'userId', width: 80 },
  {
    ...
    render(row: DataTableRowData) {
      ...
      const mimeType = row.mimeType as string;
      if (isImage(mimeType)) {
  ...
  { title: '关联ID', key: 'refId', width: 120, ellipsis: { tooltip: true } },
  { title: '关联类型', key: 'refType', width: 120 },
  { title: 'MIME类型', key: 'mimeType', width: 150 },
  {
    title: '大小',
    key: 'size',
    width: 100,
    render(row: DataTableRowData) {
      return formatFileSize(row.size as number);
    },
  },
  {
    title: '状态',
    ...
    render(row: DataTableRowData) {
      ...
      const label = status === 'ACTIVE' ? '正常' : '已删除';
  ...
  { title: '上传时间', key: 'createdAt', width: 180 },
  {
    ...
    render(row: DataTableRowData) {
      return h(
        NButton,
        { text: true, type: 'primary', size: 'small', onClick: () => window.open(row.url as string, '_blank') },
        { default: () => '下载' }
      );
    },
  },
